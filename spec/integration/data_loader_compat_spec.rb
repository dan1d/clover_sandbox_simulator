...
RSpec.describe "DataLoader DB/JSON compatibility", :db, :integration do
  ...
  # ── Full format parity: restaurant (has both DB + JSON data) ──

  context "for restaurant business type (DB vs JSON parity)" do
    before do
      CloverSandboxSimulator::Seeder.seed!(business_type: :restaurant)
    end

    let(:db_loader) { loader_class.new(business_type: :restaurant) }
    let(:json_loader) do
      allow(CloverSandboxSimulator::Database).to receive(:connected?).and_return(false)
      loader_class.new(business_type: :restaurant)
    end

    describe "categories format parity" do
      it "DB and JSON return the same keys for all categories" do
      ...
      it "first DB category exists in JSON categories" do
        first_db_name = db_loader.categories.first["name"]
        json_names = json_loader.categories.map { |c| c["name"] }

        expect(json_names).to include(first_db_name)
      end
    ...
    describe "items format parity" do
      it "both have the core keys: name, price, category" do
        %w[name price category].each do |key|
          expect(db_loader.items.first).to have_key(key), "DB item missing key '#{key}'"
          expect(json_loader.items.first).to have_key(key), "JSON item missing key '#{key}'"
        end
      end

      it "price is Integer (cents) in both" do
        (db_loader.items + json_loader.items).each do |item|
          expect(item["price"]).to be_a(Integer), "item '#{item['name']}' price is #{item['price'].class}"
          expect(item["price"]).to be > 0
        end
      end

      it "name and category are String in both" do
        (db_loader.items + json_loader.items).each do |item|
    ...
    describe "tax_rates format parity" do
      ...
      it "rate is a Numeric in both" do
        (db_loader.tax_rates + json_loader.tax_rates).each do |tr|
          expect(tr["rate"]).to be_a(Numeric), "tax rate '#{tr['name']}' rate is #{tr['rate'].class}"
        end
      end
    end
    ...
    describe "JSON fallback" do
      ...
      it "combos always come from JSON regardless of DB" do
        expect(db_loader.combos).to eq(json_loader.combos)
      end

      it "coupon_codes always come from JSON regardless of DB" do
        expect(db_loader.coupon_codes).to eq(json_loader.coupon_codes)
      end
    end

    describe "items_for_category" do
      it "returns correct items from DB path" do
        first_category = db_loader.categories.first["name"]
        items = db_loader.items_for_category(first_category)

        expect(items).not_to be_empty
        items.each { |item| expect(item["category"]).to eq(first_category) }
      end

      it "returns correct items from JSON path" do
        first_category = json_loader.categories.first["name"]
        items = json_loader.items_for_category(first_category)
  ...
  # ── Second business type: retail_clothing (DB-only, no JSON) ──

  context "for retail_clothing business type (DB path)" do
    before do
    ...
    it "loads categories from DB with correct keys" do
    ...
    it "loads items from DB with core keys matching restaurant format" do
    ...
    it "clothing items have variants" do
      items_with_variants = db_loader.items.select { |i| i.key?("variants") }
      expect(items_with_variants).not_to be_empty, "Clothing items should have variants"
    ...
    it "clothing items have SKU values" do
    ...
    it "items_for_category returns correct items" do
      first_category = db_loader.categories.first["name"]
      items = db_loader.items_for_category(first_category)
